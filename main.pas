...
if SkynetEnabled = 'true' then
  ...
  // создание OLE-объекта Excel
    ...
    mp0:=mp0+'<p style="font-family: arial, sans-serif"><b>Дата открытия уже прошла, а статус "новый"</b></p>'+sLineBreak+'<p style="font-family: arial, sans-serif">Нужно изменить статус или дату открытия<p>'+sLineBreak;
    ...
    mp0:=mp0+'<tr><th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Филиал</b></th>'+sLineBreak;
    mp0:=mp0+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Тип ОО</b></th>'+sLineBreak;
    mp0:=mp0+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Код</b></th>'+sLineBreak;
    mp0:=mp0+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Название</b></th>'+sLineBreak;
    mp0:=mp0+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 10%; background-color: tomato; color: white"><b>Статус</b></th>'+sLineBreak;
    mp0:=mp0+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Дата открытия</b></th></tr>'+sLineBreak;
     ...
     if SkynetResult[sr1,6] <> '' then
      if (SkynetResult[sr1,9] = '') and (SkynetResult[sr1,5] = 'Открыт') then
        begin
          ...
          mp3:=mp3+'<td style="border: 1px black solid; text-align: left; color: white">'+SkynetResult[sr1,9]+'_</td></tr>'+sLineBreak; // Номер телефона ОО
          mp3_flag:=true;
        end;
    mp3:=mp3+'</table>';
    ...
    mp4:=mp4+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Код</b></th>'+sLineBreak;
    mp4:=mp4+'<th style="border: 1px solid black; text-align: left; padding: 8px; width: 16%; background-color: bisque"><b>Название</b></th>'+sLineBreak;
    ...
    for sr1 := 0 to repeat_count-1 do
      begin
        ...
        mp4:=mp4+'<td style="border: 1px black solid; text-align: left">'+Repeated[sr1,2]+'</td>'+sLineBreak; // Код
        mp4:=mp4+'<td style="border: 1px black solid; text-align: left">'+Repeated[sr1,3]+'</td>'+sLineBreak; // Название
        mp4:=mp4+'<td style="border: 1px black solid; text-align: left">'+Repeated[sr1,4]+'</td>'+sLineBreak; // Статус
        mp4:=mp4+'<td style="border: 1px black solid; text-align: left">'+Repeated[sr1,5]+'</td></tr>'+sLineBreak; // Номер телефона ОО
        mp4_flag:=true;
      end;
    mp4:=mp4+'</table>';
    ...
    mp6:=mp6+'<p style="font-family: arial, sans-serif"><b>Не заполнен центр поддержки</b></p>'+sLineBreak;
...
begin
  ...
  for x := 0 to FilialsCount-1 do
    begin
       ...
       if s = SkynetFilials[x]+' ГМ' then c:=1;
       if s = SkynetFilials[x]+' МК' then c:=1;
       if s = SkynetFilials[x]+' МА' then c:=1;
    end;
